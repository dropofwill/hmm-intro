(function() {
  ...
  HMM = (function() {
    ...
    function HMM(data, cvs, matrix, unique_id) {
      ...
      this.state_loop();
      l(this.get_links_from(this.current_node.index));
    }
    ...
    HMM.prototype.setup_force = function() {
      return this.force.nodes(this.graph.nodes).links(this.graph.links).size([this.width, this.height]).alpha(0.001).linkDistance(this.link_dist).on("tick", this.tick).start();
    };
    ...
    HMM.prototype.state_loop = function() {
      l("yolo");
      if (this.current_node != null) {
        return this.draw_node(this.current_node, {
          radius: 35,
          alpha: 0.35
        });
      }
    ...
    HMM.prototype.quad_xy_at_percent = function(src, ctrl, trg, per) {
      var rev_per, x, y;
      rev_per = 1 - per;
      x = Math.pow(rev_per, 2) * src.x + 2 * rev_per * per * ctrl.x + Math.pow(per, 2) * trg.x;
      y = Math.pow(rev_per, 2) * src.y + 2 * rev_per * per * ctrl.y + Math.pow(per, 2) * trg.y;
      return new Point({
        x: x,
        y: y
      });
