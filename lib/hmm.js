(function() {
  ...
  HMM = (function() {
    ...
    HMM.prototype.setup_matrix = function(matrix_el, nodes, links) {
      var matrix, num_cols, num_rows, padded_nodes, size, sort_matrix, td, tr;
      size = nodes.length;
      num_cols = size + 2;
      num_rows = size + 1;
      padded_nodes = _(nodes).chain().unshift({
        index: ""
      }).value();
       ...
       */
      sort_matrix = _.map(_.chunk(_.sortBy(links, function(l) {
      ...
      matrix = _(sort_matrix).chain().map(function(row) {
      ...
      return td = tr.selectAll("td").data(Object).enter().append("td").text(function(d) {
        return d.prob;
      });
    };
