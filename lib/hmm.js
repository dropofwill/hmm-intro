(function() {
  ...
  HMM = (function() {
    ...
    HMM.prototype.draw_state = function() {
      var arc, node_coords, start_angle, tmp_pt;
      if (this.transitioning) {
        ...
        if (this.current_link.ctrl != null) {
          ...
          this.transition_percent -= 0.5;
          arc = this.singlenode_arc_origin(this.current_link.source);
          node_coords = new app.Point({
            x: this.current_node.x,
            y: this.current_node.y
          });
          start_angle = node_coords.sub(arc).theta;
          tmp_pt = this.circle_xy_at_percent(arc, 40, this.transition_percent, start_angle);
        }
    ...
    HMM.prototype.circle_xy_at_percent = function(src, rad, percent, start_angle) {
      var angle, x, y;
      if (start_angle == null) {
        start_angle = 0;
      }
    ...
    HMM.prototype.circle_xy_start = function(node, arc) {};
    ...
    HMM.prototype.draw_singlenode_arc = function(src, r) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(vec.x, vec.y, 5, 0, 2 * Math.PI);
      this.ctx.fill();
      this.ctx.beginPath();
      this.ctx.arc(perp1.x, perp1.y, 5, 0, 2 * Math.PI);
      this.ctx.fill();
      this.ctx.beginPath();
      this.ctx.arc(perp2.x, perp2.y, 5, 0, 2 * Math.PI);
      this.ctx.fill();
      return this.draw_cubic_curve(src, perp1, perp2, src);
    };

    HMM.prototype.singlenode_arc_origin = function(src, r) {
      var pt, vec;
      if (r == null) {
        r = 40;
      }
      ...
      return vec = pt.sub(this.center).normalize().mul(r).add(pt);
    };
